  ...
  if (!result) {
    return (
      <div className="container" style={{padding: '100px', textAlign: 'center'}}>
        <h2 style={{color: '#e74c3c'}}>{t('invalid_qr')}</h2>
    ...
    <div className="container" style={{ padding: '60px 20px', maxWidth: '600px' }}>
      <div className="glass-card" style={{ padding: '40px', textAlign: 'center', borderTop: result.valid ? '10px solid #2ecc71' : '10px solid #e74c3c' }}>
        ...
        <h2 style={{ fontSize: '32px', marginBottom: '10px' }}>
          {result.valid ? t('ticket_valid') : t('ticket_invalid_status')}
        </h2>
        
        {result.details && (
          <div style={{ textAlign: 'left', marginTop: '30px', padding: '20px', background: 'rgba(255,255,255,0.05)', borderRadius: '12px' }}>
            <p><strong>{t('visitor')}:</strong> {result.details.user_name}</p>
            <p><strong>{t('event')}:</strong> {result.details.title}</p>
            <p><strong>{t('date_time')}:</strong> {result.details.session_date || result.details.date} | {result.details.session_time || result.details.time}</p>
            <p><strong>{t('location')}:</strong> {result.details.session_city || result.details.city}, {result.details.session_location || result.details.location}</p>
            <p><strong>{t('ticket_count_label')}:</strong> {result.details.tickets}</p>
            <p><strong>{t('status')}:</strong> <span style={{ color: result.valid ? '#2ecc71' : '#e74c3c', fontWeight: 'bold' }}>{result.details.status}</span></p>
          </div>
        )}
